...
def example_ml_policy(basic_market: Market) -> MLPolicy:  # noqa: F811
    ...
    policy = MLPolicy(
        ...
        n_stocks_to_sample=5,
        max_stocks_per_timestep=10,
        valuation_model_path=TEST_MODULE / "models" / "random_forest_regressor.pkl",
    )
    policy.initialize_portfolio(example_portfolio)
    return policy
...
def test_ml_policy_infer_buy_actions_completes(example_ml_policy) -> None:
    # Act.
    buy_actions = example_ml_policy.infer_buy_actions()
...
def test_ml_policy_infer_sell_actions_completes(example_ml_policy) -> None:
    # Act.
    sell_actions = example_ml_policy.infer_sell_actions()
